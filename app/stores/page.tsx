...
function StoresPageContent() {
  const { brandIds: brands, radiusKm: radius, setBrandIds: setBrands, setRadiusKm: setRadius } = useSearchPreferences();
  ...
  return <PageFrame>
    <section className="container-wide py-8 md:py-12">
      ...
      <div className="surface mt-7 rounded-3xl p-4 md:p-6">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-[var(--border)] pt-4"><SlidersHorizontal size={17} className="mr-1 text-[var(--muted)]" />{facilityOptions.map(([id, label]) => <button key={id} className="brand-chip" aria-pressed={facilities.includes(id)} style={facilities.includes(id) ? { background: "var(--green)" } : {}} onClick={() => toggleFacility(id)}>{label}</button>)}</div><p className="mt-3 text-xs text-[var(--muted)]">ข้อมูลเวลาเปิด ที่จอดรถ ATM และร้านในปั๊มอ้างอิงจาก OpenStreetMap ซึ่งอาจไม่ครบทุกสาขา</p>
      </div>
