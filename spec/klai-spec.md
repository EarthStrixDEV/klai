...
> ⚠️ **ยังไม่ได้ verify สด**: tag `brand` ของ Inthanin, Punthai, Black Canyon และ Chao Doi ใน OSM ยังไม่ได้ตรวจสอบจริง (สภาพแวดล้อมตอนคุยกันนี้เรียก Overpass API ตรงๆ ไม่ได้) พี่เอิร์ธควรลองเช็คค่า `brand`/`name` ที่แต่ละสาขาใช้จริงผ่าน [overpass-turbo.eu](https://overpass-turbo.eu) ก่อนเขียนเป็น config จริง เผื่อบาง node ใส่แค่ `name` ไม่มี `brand` ทำให้กรองไม่เจอทั้งที่มีอยู่
...
แสดงระยะทาง, สิ่งอำนวยความสะดวก (EV charger, ห้องน้ำ, ที่จอดรถ), เวลาเปิด-ปิด — **Black Canyon และ Chao Doi ไม่เข้าโมดูลนี้** เพราะไม่มีความสัมพันธ์กับปั๊มน้ำมันแบรนด์ใดแบรนด์หนึ่งแบบผูกขาด (พบทั่วไปในห้าง/อาคารมากกว่าปั๊ม) — ยังค้นหาได้ปกติผ่าน Map/List Store
...
- ไม่ใช้โลโก้จริงของแบรนด์ไหนทั้งสิ้น (7-Eleven, PTT, บางจาก, PT, Café Amazon ฯลฯ) — ใช้ชื่อ/สีอ้างอิงเป็นข้อความล้วนเท่านั้น พร้อม disclaimer ครอบคลุมทุกแบรนด์ในหน้า About

## 10. Open Questions

- **Tag `brand` จริงของ Inthanin/Punthai/Black Canyon/Chao Doi ใน OSM** — ยังไม่ได้ verify สด ต้องเช็คผ่าน overpass-turbo.eu ก่อนใส่ใน Brand Registry จริง (ดูหัวข้อ 4)
