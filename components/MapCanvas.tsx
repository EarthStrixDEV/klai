...
export default function MapCanvas({ center, stores, radiusKm, selectedId, onSelect }: { center: Coordinates; stores: Store[]; radiusKm: number; selectedId?: string; onSelect?: (store: Store) => void }) {
  return <MapContainer key={`${center.lat}-${center.lng}`} center={[center.lat, center.lng]} zoom={radiusKm <= 1 ? 15 : 13} scrollWheelZoom>
    ...
    {stores.map((store) => <Marker key={store.id} position={[store.lat, store.lng]} icon={icon(store, selectedId === store.id)} eventHandlers={{ click: () => onSelect?.(store) }}><Popup><strong>{store.name}</strong><br />{formatDistance(store.distanceKm)}<br /><a href={navigationUrl(store)} target="_blank">นำทาง</a></Popup></Marker>)}
  </MapContainer>;
}
