...
export default function ShoppingListPage() {
  ...
  useEffect(() => { if (ready) try { localStorage.setItem(STORAGE_KEY, JSON.stringify(items)); } catch { queueMicrotask(() => setNotice("บันทึกลงเครื่องไม่ได้ ลิสต์จะอยู่จนกว่าจะปิดแท็บ")); } }, [items, ready]);
  ...
  return <PageFrame>
    <section className="container-wide max-w-4xl py-8 md:py-12">
      <div className="flex items-end justify-between gap-4"><div><p className="eyebrow">วางแผนก่อนแวะ</p><h1 className="display-font mt-1 text-3xl font-extrabold">ลิสต์ของฉัน</h1></div>{items.length > 0 && <button onClick={() => { if (window.confirm("ล้างลิสต์ทั้งหมดใช่ไหม? การทำรายการนี้ย้อนกลับไม่ได้")) dispatch({ type: "clear" }); }} className="text-sm font-bold text-[var(--muted)] hover:text-red-600">ล้างลิสต์</button>}</div>
      ...
      <div className="mt-6 space-y-7">{Object.entries(categories).map(([categoryId, categoryLabel]) => { const categoryItems = items.filter((item) => item.category === categoryId); if (!categoryItems.length) return null; return <section key={categoryId}><h2 className="display-font mb-3 text-sm font-bold text-[var(--green-dark)]">{categoryLabel} <span className="font-normal text-[var(--muted)]">({categoryItems.length})</span></h2><div className="space-y-3">{categoryItems.map((item) => <article key={item.id} className={`surface flex items-center gap-3 rounded-2xl p-4 ${item.completed ? "opacity-60" : ""}`}>
